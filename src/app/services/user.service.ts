...
export class UserService {
  ...
  updateUserName(dto: UserUpdateDto) {
    return this.http.put(environment.apiUrl + this.apiControllerName + '/UserName', dto).subscribe(
      () => {},
      error => {
        this.notifyService.notification$.next({message:'Błąd zmiany loginu', isError: true});
        console.error(error);
      },
      () => {
        this.notifyService.notification$.next({message: 'Zmiana loginu przebiegła pomyślnie', isError: false});
      }
      );
  }
